...
min_locations = zeros(Float64, length(lattice_sizes))
max_locations = zeros(Float64, length(lattice_sizes))
...
for Lidx=eachindex(lattice_sizes)
    ...
    min_range = (min_peak[2]-1):(min_peak[2]+1)
    max_range = (max_peak[2]-1):(max_peak[2]+1)
    ...
    min_locations[Lidx] = parabola_extrema(fit_min.param)
    max_locations[Lidx] = parabola_extrema(fit_max.param)

    lines!(axs, temps[min_range[1]]:0.0001:temps[min_range[end]], parabola(temps[min_range[1]]:0.0001:temps[min_range[end]], fit_min.param), color=:grey69, label="L = $L")
    lines!(axs, temps[max_range[1]]:0.0001:temps[max_range[end]], parabola(temps[max_range[1]]:0.0001:temps[max_range[end]], fit_max.param), color=:grey69, label="L = $L")
...
scatterlines!(ax, inv.(lattice_sizes), max_locations .- Tc_inf, color=:red, label="Maxima")
scatterlines!(ax, inv.(lattice_sizes), min_locations .- Tc_inf, color=:blue, label="Minima")
