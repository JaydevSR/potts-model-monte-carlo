...
for L in Lvals
    ...
    @floop ThreadedEx(basesize = length(temps) ÷ 4) for i in eachindex(temps)
        T = temps[i]
        ...
        order_k = 2
        suzzs[i] = suzz_kth(mag_x_arr, T, L*L, order_k)
        err_suzzs[i] = bootstrap_err(mag_x_arr, suzz_kth, T, L*L, order_k; r=bootstrap_samples)
