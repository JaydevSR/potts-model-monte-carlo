...
for r in rng
    ...
    f1!(F, x) = residual!(F, x; ratio=ratio, r=r, L=lattice_size, i=power_i)
    initial_x = [1.0]

    soln1 = nlsolve(f1!, initial_x, autodiff=:forward, ftol=1e-10, iterations=10_000)

    mvals1[r] = soln1.zero[1]
...
scatter!(ax, rng, mvals1, marker=:xcross, markersize=16)
display(fig)
